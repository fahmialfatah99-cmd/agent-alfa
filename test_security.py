...
class TestSafeToolsSubset:
    ...
    def test_tidak_ada_tools_berisiko(self):
        import main_brain
        names = {t["function"]["name"] for t in main_brain.build_openai_tools(safe_only=True)}
        assert not (names & FORBIDDEN_IN_SAFE), f"tools berisiko bocor: {names & FORBIDDEN_IN_SAFE}"
...
class TestPreciseEditing:
    ...
    def test_edit_tidak_ditemukan_memberi_hint(self, tmp_path):
        ...
        assert r["status"] == "error" and ("Kemungkinan" in r["message"] or "tidak ditemukan" in r["message"])
...
class TestStructuralUpgrades:
    ...
    def test_tool_baru_terdaftar_di_available_tools(self):
        ...
        for wajib in ("edit_file_precise", "apply_unified_diff",
    ...
    def test_search_codebase_masuk_subset_aman_swarm(self):
        ...
        names = {t["function"]["name"]
                 for t in main_brain.build_openai_tools(safe_only=True)}
