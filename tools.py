...
from runtime_ctx import current_chat_id_var as current_chat_id_var  # noqa: E402
from runtime_ctx import current_user_id_var as current_user_id_var
from runtime_ctx import get_current_chat_id as get_current_chat_id
from runtime_ctx import get_current_user_id as get_current_user_id
...
def get_system_stats() -> Dict[str, Any]:
    ...
    try:
        ...
        top_cpu = sorted(
            processes, key=lambda p: p.get("cpu_percent") or 0, reverse=True
        )[:4]
...
def _bash_blocked_reason(command: str) -> Optional[str]:
    ...
    if m:
        ...
        has_danger_target = bool(
            _re.search(r"(?:^|\s)(\"|')?" + _RM_DANGER_TARGETS, seg)
        )
...
def execute_bash_command(
    ...
    use_docker = (
        pref in ("auto", "docker") and _docker_available() and _ensure_sandbox_image()
    )
    ...
    try:
        if use_docker:
            ...
            home_in_box = (
                "/workspace" if (wd_abs and os.path.isdir(wd_abs)) else "/sandbox"
            )
            cmd = [
                "docker",
                "run",
            ...
            for pdir in project_dirs:
                if os.path.isdir(pdir) and pdir != wd_abs and pdir != tf:
                    cmd += [
                        ...
                        f"{pdir}:{pdir}",
                        "-v",
                        f"{pdir}:{home_in_box}/{os.path.basename(pdir)}",
                    ]
            ...
            for pdir in project_dirs:
                if os.path.isdir(pdir):
                    cmd += [
                        ...
                        f"{pdir}:{pdir}",
                        "-v",
                        f"{pdir}:/sandbox/{os.path.basename(pdir)}",
                    ]
    ...
    try:
        ...
        with httpx.Client(timeout=15.0, follow_redirects=True) as client:
            ...
            if resp.status_code != 200:
                return {
                    "status": "error",
                    "message": f"Gagal mengambil URL, status code: {resp.status_code}",
                }
            ...
            return {
                ...
                "url": url,
                "content": text
                or "Halaman web tidak memuat konten teks yang dapat dibaca.",
            }
    except Exception as e:
...
def save_knowledge_memory(
    ...
    try:
        ...
        msg = database.save_memory_fact_sync(
            user_id=user_id, key_topic=key_topic, content=content, category=category
        )
...
def schedule_reminder(reminder_time_iso: str, message: str) -> Dict[str, Any]:
    ...
    try:
        ...
        rem_id = database.add_reminder_sync(
            user_id=user_id,
            chat_id=chat_id,
            reminder_time_iso=reminder_time_iso,
            message=message,
        )
        return {
            "status": "success",
...
def pdf_merge_documents(
    ...
    try:
        ...
        if merged_count == 0:
            return {
                "status": "error",
                "message": "Tidak ada file PDF valid yang ditemukan untuk digabungkan.",
            }
...
def pdf_split_document(
    ...
    try:
        ...
        if not os.path.exists(exp_p):
            return {
                "status": "error",
                "message": f"File PDF '{pdf_path}' tidak ditemukan.",
            }
        ...
        target_dir = (
            ...
            if output_dir
            else os.path.join(get_pdf_output_dir("Split"), base_stem)
        )
        os.makedirs(target_dir, exist_ok=True)
...
def pdf_extract_full_text(pdf_path: str, page_numbers: str = "") -> Dict[str, Any]:
    ...
    try:
            ...
            if page_numbers:
                ...
                for p in page_numbers.split(","):
                    ...
                    if "-" in p:
                        s, e = p.split("-", 1)
                        req_indices.extend(
                            range(max(0, int(s) - 1), min(total_pages, int(e)))
                        )
                    elif p.isdigit():
                        idx = int(p) - 1
        ...
        try:
            ...
            return {
                "status": "error",
                "message": f"Gagal ekstrak teks PDF: {str(err2)}",
            }
...
def pdf_encrypt_password(
    ...
    try:
        ...
        writer.encrypt(
            user_password=password, owner_password=password, algorithm="AES-256"
        )
        with open(target_path, "wb") as f_out:
            writer.write(f_out)
    ...
    try:
        ...
        if reader.is_encrypted:
            ...
            if not res:
                return {
                    "status": "error",
                    "message": "Password salah atau PDF tidak dapat didekripsi.",
                }
...
def pdf_compress_and_optimize(
    ...
    try:
        ...
        savings_pct = (
            max(0, round((orig_size - new_size) / orig_size * 100, 1))
            if orig_size > 0
            else 0
        )
...
def affiliate_hunt_trending_products(
    niche: str = "gadget unik murah", platform: str = "shopee"
) -> Dict[str, Any]:
    """
...
def marketplace_search_products(
    ...
    try:
        ...
        return fast_scraper.search_and_scrape_marketplace(
            query=query, platform=platform, max_items=max_items
        )
    except Exception as e:
...
def control_linux_hardware(action: str, value: str = "") -> Dict[str, Any]:
    ...
    try:
        ...
        if act == "lock_screen":
            ...
            return {
                "status": "success",
                "message": "🔒 Layar desktop Linux telah berhasil dikunci.",
            }
            ...
            return {
                "status": "success",
                "message": f"🔊 Volume speaker berhasil diubah ke {vol_val}%.",
            }
            ...
            return {
                "status": "success",
                "message": "🔇 Status mute audio berhasil dialihkan (toggle).",
            }
            ...
            res = subprocess.run(
                "nmcli -f SSID,SIGNAL,SECURITY dev wifi list | head -n 12",
                shell=True,
                capture_output=True,
                text=True,
                timeout=8,
            ...
            return {
                "status": "success",
                "wifi_networks": res.stdout.strip()
                or "Tidak ada jaringan Wi-Fi ditemukan.",
            }

        elif act == "bluetooth_status":
            res = subprocess.run(
                "bluetoothctl show 2>/dev/null | grep -E 'Name|Powered|Discoverable'; bluetoothctl devices 2>/dev/null",
                shell=True,
                capture_output=True,
                text=True,
                timeout=5,
            ...
            return {
                "status": "success",
                "bluetooth": res.stdout.strip() or "Bluetooth tidak aktif.",
            }
            ...
            if not bat:
                return {
                    "status": "success",
                    "battery": "Perangkat tidak menggunakan baterai (Desktop PC / Server).",
                }
            plugged = (
                "🔌 Sedang Mengisi Daya (Charging)"
                if bat.power_plugged
                else "🔋 Berjalan dengan Baterai (Discharging)"
            )
            time_left = (
                f"{round(bat.secsleft / 60)} menit" if bat.secsleft > 0 else "N/A"
            )
            return {
                "status": "success",
        ...
        return {
            "status": "error",
            "message": f"Aksi hardware '{action}' tidak dikenal.",
        }
    except Exception as e:
        return {"status": "error", "message": str(e)}
...
def read_clipboard() -> Dict[str, Any]:
    ...
    try:
        if os.name == "nt":
            ...
            res = subprocess.run(
                "wl-paste 2>/dev/null || xclip -selection clipboard -o 2>/dev/null || xsel --clipboard --output 2>/dev/null",
                shell=True,
                capture_output=True,
                text=True,
                timeout=3,
    ...
    try:
        if os.name == "nt":
            ...
            if res.returncode == 0:
                return {
                    "status": "success",
                    "message": f"Teks berhasil disalin ke clipboard ({len(text)} karakter). Siap di-paste (Ctrl+V).",
                }
            return {
                "status": "error",
                "message": f"Set-Clipboard gagal: {(res.stderr or '')[:200]}",
            }
...
def show_desktop_notification(
    ...
    try:
        ...
        return {
            "status": "success",
            "message": f"Notifikasi desktop '{title}' berhasil ditampilkan di layar.",
        }
    except Exception as e:
        return {"status": "error", "message": str(e)}
...
def query_database(db_path: str, sql_query: str) -> Dict[str, Any]:
    ...
    try:
        ...
        if not os.path.exists(expanded):
            return {
                "status": "error",
                "message": f"Database file tidak ditemukan: {db_path}",
            }
        ...
        if (
            ...
            return {
                ...
                "total_available": (
                    len(rows)
                    if len(rows) <= 100
                    else f"{len(rows)}+ (showing first 100)"
                ),
                "data": data,
            }
        else:
            ...
            return {
                "status": "success",
                "message": f"Query berhasil dieksekusi. {affected} baris terpengaruh.",
            }
    except Exception as e:
...
def send_email(
    ...
    try:
        ...
        if not smtp_email or not smtp_password:
            return {
                "status": "error",
                "message": "SMTP_EMAIL dan SMTP_PASSWORD belum dikonfigurasi di file .env. Tambahkan: SMTP_EMAIL=xxx@gmail.com dan SMTP_PASSWORD=your_app_password",
            }
...
def kill_process(pid_or_name: str) -> Dict[str, Any]:
    ...
    try:
        ...
        if killed:
            return {
                "status": "success",
                "message": f"Proses berhasil dihentikan: {', '.join(killed)}",
            }
        return {
            "status": "error",
            "message": f"Proses '{pid_or_name}' tidak ditemukan.",
        }
    except psutil.NoSuchProcess:
        return {
            "status": "error",
            "message": f"Proses dengan PID/nama '{pid_or_name}' tidak ditemukan.",
        }
    except psutil.AccessDenied:
        return {
            "status": "error",
            "message": "Akses ditolak. Coba jalankan dengan sudo.",
        }
    except Exception as e:
        return {"status": "error", "message": str(e)}
...
def deep_research_topic(topic: str, max_depth: int = 3) -> Dict[str, Any]:
    ...
    try:
                ...
                try:
                    ...
                    for r in results:
                        ...
                        if (
                            ...
                            and not u.endswith((".pdf", ".exe", ".zip", ".png", ".jpg"))
                        ):
                            seen_urls.add(u)
                            sources_data.append(
                                {
        ...
        if not sources_data:
            return {
                "status": "error",
                "message": f"Tidak ditemukan sumber riset untuk topik: '{topic}'",
            }
...
def auto_diagnose_and_heal_system(fix_issues: bool = False) -> Dict[str, Any]:
    ...
    try:
        ...
        failed_user_units = [
            line.strip().split()[0]
            for line in res_failed.stdout.strip().splitlines()
            if line.strip()
        ]
        ...
        res_journal = subprocess.run(
            "journalctl --user -p 3 -n 15 --no-pager 2>/dev/null",
            shell=True,
            capture_output=True,
            text=True,
            timeout=5,
        )
        diagnosis["recent_critical_logs"] = (
            res_journal.stdout.strip()[:1500]
            if res_journal.stdout.strip()
            else "Tidak ada critical error log terbaru."
        )
        ...
        disk = psutil.disk_usage("/")
        swap = psutil.swap_memory()
        diagnosis["disk_health"] = (
            f"Root: {disk.percent}% used ({round(disk.free / (1024**3), 1)} GB free)"
        )
        ...
        if fix_issues:
            ...
            for unit in failed_user_units:
                if (
                    "telegram-ai-bot" not in unit
                ):  # avoid recursive restart in diagnostic turn
                    subprocess.run(
            ...
            if disk.percent > 85:
                subprocess.run(
                    "journalctl --user --vacuum-time=2d", shell=True, timeout=10
                )
                healing_actions.append("Cleaned old user journal logs")

            diagnosis["healing_executed"] = (
                healing_actions
                if healing_actions
                else "Tidak ada tindakan perbaikan yang diperlukan saat ini."
            )

        return {"status": "success", "diagnosis": diagnosis, "fix_mode": fix_issues}
    except Exception as e:
...
def clean_system_storage(dry_run: bool = True) -> Dict[str, Any]:
    ...
    try:
        ...
        return {
            ...
            "details": report,
            "message": (
                f"{action_msg}: Potensi ruang dibersihkan: {round(total_freed_mb, 2)} MB. Jalankan dengan dry_run=False untuk eksekusi pembersihan nyata."
                if dry_run
                else f"Pembersihan berhasil! {round(total_freed_mb, 2)} MB ruang disk berhasil dikembalikan."
            ),
        }
    except Exception as e:
...
def manage_crontab_jobs(
    ...
    try:
        ...
        if action == "list":
            ...
            return {
                "status": "success",
                "crontab_entries": entries if entries else "(Crontab kosong)",
            }
        elif action == "add":
            if not cron_line:
                return {
                    "status": "error",
                    "message": "Parameter cron_line harus diisi.",
                }
            ...
            proc = subprocess.Popen(
                "crontab -", shell=True, stdin=subprocess.PIPE, text=True
            )
            proc.communicate(input=new_cron, timeout=5)
            return {
                "status": "success",
                "message": f"Entri crontab berhasil ditambahkan: '{cron_line}'",
            }
        elif action == "remove":
            if not search_pattern:
                return {
                    "status": "error",
                    "message": "Parameter search_pattern harus diisi untuk menghapus entri crontab.",
                }
            res_curr = subprocess.run(
                "crontab -l 2>/dev/null",
                shell=True,
            ...
            lines = [
                ln for ln in res_curr.stdout.splitlines() if search_pattern not in ln
            ]
            new_cron = "\n".join(lines).strip() + "\n"
            proc = subprocess.Popen(
                "crontab -", shell=True, stdin=subprocess.PIPE, text=True
            ...
            return {
                "status": "success",
                "message": f"Entri crontab yang cocok dengan pola '{search_pattern}' berhasil dihapus.",
            }
        return {
            "status": "error",
            "message": f"Aksi '{action}' tidak dikenal. Gunakan: list, add, remove.",
        }
    except Exception as e:
        return {"status": "error", "message": f"Crontab error: {str(e)}"}


def extract_and_link_knowledge(
    entity: str,
    relation: str,
    target_value: str,
    ...
    return database.add_knowledge_relation_sync(
        uid, entity, relation, target_value, category, tags
    )
    ...
    try:
        ...
        if fmt == "json":
            ...
            for f in data.get("facts", []):
                md_lines.append(
                    f"• **[{f['category'].upper()}] {f['key_topic']}**: {f['content']}"
                )
            ...
            for r in data.get("knowledge_graph", []):
                ...
                md_lines.append(
                    f"• **{r['entity']}** ──({r['relation']})──> **{r['target_value']}** ({r['category']}){tag_str}"
                )
...
def libreoffice_convert_document(
    ...
    try:
        ...
        return {
            "status": "error",
            "message": f"Konversi LibreOffice gagal: {res.stderr or res.stdout}",
        }
    except Exception as e:
...
def libreoffice_render_page_previews(
    ...
    try:
        ...
        if not os.path.exists(expanded):
            return {
                "status": "error",
                "message": f"File dokumen tidak ditemukan: {document_path}",
            }
        ...
        if ext == ".pdf":
            ...
            if not os.path.exists(temp_pdf):
                return {
                    "status": "error",
                    "message": "Gagal mengonversi dokumen ke PDF untuk rendering preview.",
                }
            pdf_path = temp_pdf
        ...
        return {
            "status": "error",
            "message": f"Gagal merender halaman: {res_ppm.stderr}",
        }
    except Exception as e:
        return {
            "status": "error",
            "message": f"LibreOffice render preview error: {str(e)}",
        }


def libreoffice_create_document(
    doc_type: str,
    title: str,
...
<div class="footer">Dibuat secara otomatis oleh Sovereign Telegram AI Agent via LibreOffice Engine | {datetime.datetime.now().strftime("%d %B %Y %H:%M")}</div>
        ...
        return {
            "status": "error",
            "message": f"Gagal membuat dokumen LibreOffice: {res.stderr or res.stdout}",
        }
    except Exception as e:
        return {
            "status": "error",
            "message": f"Create LibreOffice document error: {str(e)}",
        }
...
def libreoffice_extract_document_text(document_path: str) -> Dict[str, Any]:
    ...
    try:
        ...
        return {
            "status": "error",
            "message": "Tidak dapat mengekstrak teks dari dokumen via LibreOffice.",
        }
    except Exception as e:
        return {
            "status": "error",
            "message": f"LibreOffice extract text error: {str(e)}",
        }
...
def semantic_search_vector_brain(
    query: str, top_k: int = 5, category: str = ""
) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        results = vector_memory.semantic_search(
            user_id=user_id, query=query, top_k=top_k, category=category or None
        )
        return {
            "status": "success",
            "query": query,
...
def ingest_document_to_vector_brain(
    title: str, content_or_file_path: str, category: str = "general"
) -> Dict[str, Any]:
    """
...
def markitdown_convert_document(
    ...
    try:
        ...
        return {
            ...
            or os.path.basename(source_path_or_url),
    ...
    try:
        if action == "status":
            ...
            with open(shot_file, "wb") as f:
                subprocess.run(
                    [adb_bin] + dev_flag + ["exec-out", "screencap", "-p"],
                    stdout=f,
                    timeout=10,
                )
            return {
                ...
                "file_size": (
                    os.path.getsize(shot_file) if os.path.exists(shot_file) else 0
                ),
            }
            ...
            subprocess.run(
                [adb_bin] + dev_flag + ["shell", "input", "keyevent", keycode],
                capture_output=True,
                timeout=5,
            )
            ...
            if len(coords) < 2:
                return {
                    "status": "error",
                    "message": "Koordinat tap harus berupa 'X Y' (contoh: '500 800')",
                }
            subprocess.run(
                [adb_bin] + dev_flag + ["shell", "input", "tap", coords[0], coords[1]],
                capture_output=True,
                timeout=5,
            )
            ...
            if not scrcpy_bin:
                return {
                    "status": "error",
                    "message": "Binary scrcpy tidak ditemukan. Install dengan 'sudo apt install -y scrcpy'",
                }
            subprocess.Popen([scrcpy_bin] + (["-s", device_id] if device_id else []))
            return {
                "status": "success",
                "message": "Window screen mirroring Scrcpy berhasil dibuka di desktop.",
            }
...
from gdrive_suite import _detect_gdrive_auth_mode as _detect_gdrive_auth_mode
from gdrive_suite import _get_default_gdrive_folder_id as _get_default_gdrive_folder_id
from gdrive_suite import _get_gdrive_service as _get_gdrive_service
from gdrive_suite import gdrive_create_folder as gdrive_create_folder
from gdrive_suite import gdrive_download_file as gdrive_download_file
from gdrive_suite import gdrive_list_files as gdrive_list_files
from gdrive_suite import gdrive_oauth_exchange_code as gdrive_oauth_exchange_code
from gdrive_suite import gdrive_oauth_get_auth_url as gdrive_oauth_get_auth_url
from gdrive_suite import gdrive_oauth_login as gdrive_oauth_login
from gdrive_suite import gdrive_oauth_logout as gdrive_oauth_logout
from gdrive_suite import (
    gdrive_save_oauth_client_secret as gdrive_save_oauth_client_secret,
)
from gdrive_suite import gdrive_status as gdrive_status
from gdrive_suite import gdrive_sync_to_second_brain as gdrive_sync_to_second_brain
from gdrive_suite import gdrive_upload_file as gdrive_upload_file
from git_sandbox import git_worktree_sandbox_create as git_worktree_sandbox_create
from git_sandbox import git_worktree_sandbox_list as git_worktree_sandbox_list
from git_sandbox import git_worktree_sandbox_rollback as git_worktree_sandbox_rollback
...
def self_restart_service() -> Dict[str, Any]:
    ...
    try:
        ...
        tools_path = os.path.join(
            os.path.dirname(os.path.abspath(__file__)), "tools.py"
        )
        ...
        if compile_check.returncode != 0:
            return {
                "status": "error",
                "message": f"Tidak bisa restart — tools.py memiliki error: {compile_check.stderr}",
            }
        ...
        if compile_check2.returncode != 0:
            return {
                "status": "error",
                "message": f"Tidak bisa restart — bot.py memiliki error: {compile_check2.stderr}",
            }
...
def proactive_system_guardian_config(
    action: str = "status",
    ...
    try:
        config_path = os.path.join(
            os.path.expanduser("~"), ".alfa", "guardian_config.json"
        )
        ...
        if action == "status":
            ...
            return {
                ...
                "guardian": {
                    "enabled": False,
                    "message": "Guardian belum dikonfigurasi.",
                },
            }
        ...
        return {
            "status": "error",
            "message": f"Action '{action}' tidak dikenal. Gunakan: status, enable, disable.",
        }
    except Exception as e:
        return {"status": "error", "message": str(e)}


def proactive_ambient_agent_config(
    action: str = "status",
    enabled: bool = True,
    ...
    try:
        ...
        if action == "status":
            ...
            return {
                "status": "success",
                "message": "🤖 Mode Proaktif Otonom dinonaktifkan. Bot hanya akan membalas jika Anda bertanya.",
            }
    ...
    try:
        ...
        if act == "status":
            ...
            res_status = subprocess.run(
                ["systemctl", "--user", "status", svc_name, "--no-pager", "-n", "5"],
                capture_output=True,
                text=True,
            )
            ...
            res = subprocess.run(
                ["systemctl", "--user", act, svc_name], capture_output=True, text=True
            )
            if res.returncode == 0:
                time.sleep(1)
                res_active = subprocess.run(
            ...
            return {
                "status": "error",
                "message": f"Gagal mengeksekusi {act}: {res.stderr}",
            }

        elif act == "logs":
            res_logs = subprocess.run(
                ["journalctl", "--user", "-u", svc_name, "-n", "25", "--no-pager"],
                capture_output=True,
                text=True,
            )
            return {
        ...
        return {
            "status": "error",
            "message": f"Aksi '{action}' tidak dikenal. Gunakan: status, start, stop, restart, logs, enable.",
        }
    except Exception as e:
...
def manage_api_keys(
    ...
    try:
        if action == "list":
            ...
            return {
                "status": "error",
                "message": f"Action tidak dikenal: {action}. Gunakan 'list', 'add', 'activate', atau 'delete'.",
            }
    except Exception as e:
...
def list_wa_drive_uploads(limit: int = 20) -> Dict[str, Any]:
    ...
    try:
        ...
        compact = [
            {
                "waktu": u.get("ts"),
                "nama_file": u.get("file_name"),
                "folder": u.get("folder"),
                "pengirim": u.get("sender"),
                "grup": u.get("group"),
                "link": u.get("web_link"),
            }
            for u in uploads
