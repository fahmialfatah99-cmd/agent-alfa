...
def test_tool_rag_synonyms_and_intent():
    ...
    ranked_pdf = tool_rag._rank_names(
        docs=[
            "pdf_merge_documents combine pdf",
            "execute_bash_command run bash",
            "edit_image crop photo",
        ],
    ...
    ranked_sys = tool_rag._rank_names(
        docs=[
            "get_system_stats check cpu ram",
            "kill_process terminate pid",
            "pdf_merge_documents combine pdf",
        ],
...
def test_self_heal_hint_generation():
    ...
    hint1 = tools.generate_self_heal_hint(
        "execute_bash_command", "", stderr="bash: foobar_cmd: command not found"
    )
    assert hint1 is not None
    assert "[SELF_HEAL_HINT]" in hint1
    assert "tidak ditemukan" in hint1

    hint2 = tools.generate_self_heal_hint(
    ...
    hint3 = tools.generate_self_heal_hint(
        "execute_python_sandbox", "", stderr="SyntaxError: invalid syntax"
    )
