                ...
                for attr_name in dir(module):
                    ...
                    if callable(attr) and hasattr(attr, "__doc__") and attr.__doc__:
                        if getattr(attr, "__module__", "") == module_name:
                            ...
                            logger.info(
                                f"Loaded dynamic plugin helper: {attr_name} from {basename}"
                            )
        except Exception as e:
...
def create_and_register_plugin(
    ...
    try:
        ...
        res = subprocess.run(
            [sys.executable, "-m", "py_compile", plugin_path],
            capture_output=True,
            text=True,
        )
        if res.returncode != 0:
            ...
            return {
                "status": "error",
                "message": f"Plugin compilation failed: {res.stderr}",
            }
        ...
        if not target_fn:
            ...
            return {
                "status": "error",
                "message": f"Plugin tidak mendefinisikan fungsi callable '{clean_name}'.",
            }
        ...
        if test_kwargs is not None and isinstance(test_kwargs, dict):
            try:
                ...
                logger.warning(
                    f"Plugin sandbox test failed with kwargs {test_kwargs}: {run_err}"
                )
        ...
        params_info = {
            p.name: {
                "default": (
                    str(p.default) if p.default != inspect.Parameter.empty else None
                ),
                "annotation": (
                    str(p.annotation)
        ...
        logger.info(
            f"🧬 Dynamic plugin '{clean_name}' successfully compiled, tested, and hot-loaded!"
        )
...
def list_all_plugins() -> List[Dict[str, Any]]:
    ...
    try:
        ...
        rows = conn.execute(
            "SELECT * FROM dynamic_plugins ORDER BY created_at DESC"
        ).fetchall()
        for r in rows:
            plugins_list.append(
                {
                    ...
                    "file_exists": os.path.exists(
                        os.path.join(PLUGINS_DIR, f"{r['tool_name']}.py")
                    ),
                }
            )
    except Exception as e:
...
def execute_plugin_direct(tool_name: str, **kwargs) -> Any:
    ...
    if not fn:
        raise ValueError(
            f"Dynamic plugin '{clean_name}' tidak ditemukan atau gagal dimuat."
        )
