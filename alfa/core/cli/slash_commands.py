...
class CliSlashCommandsMixin:
    ...
    def do_slash_provider(self, arg):
        ...
        if not arg:
            ...
            if res and res.status_code == 200:
                ...
                print(
                    f"  {Colors.CYAN}Provider:{Colors.ENDC} {data.get('provider', 'Not set')}"
                )
                print(
                    f"  {Colors.CYAN}Model:{Colors.ENDC} {data.get('model', 'Not set')}"
                )
                print(
                    f"  {Colors.CYAN}Temperature:{Colors.ENDC} {data.get('temperature', 0.7)}"
                )
                print(
                    f"  {Colors.CYAN}Max Tokens:{Colors.ENDC} {data.get('max_tokens', 4096)}"
                )
                print()
            else:
        ...
        if provider not in providers:
            print_status(
                f"Provider tidak valid. Pilihan: {', '.join(providers)}", "error"
            )
            return
    ...
    def do_slash_settings(self, arg):
        ...
        if res and res.status_code == 200:
            print_status(
                f"✅ Settings diperbarui: {', '.join(settings.keys())}", "success"
            )
        else:
    ...
    def do_slash_switch(self, arg):
        ...
        presets = {
            ...
            "creative": {
                "temperature": 0.9,
                "max_tokens": 4096,
                "description": "High creativity, more varied",
            },
            "precise": {
                "temperature": 0.1,
                "max_tokens": 2048,
                "description": "Very precise, deterministic",
            },
            "coding": {
                "temperature": 0.2,
                "max_tokens": 8192,
        ...
        if res and res.status_code == 200:
            ...
            print_status(
                f"   Temperature: {preset['temperature']}, Max Tokens: {preset['max_tokens']}",
                "info",
            )
        else:
    ...
    def do_slash_agents(self, arg):
        ...
        if res and res.status_code == 200:
            ...
            for agent in agents:
                ...
                icon = (
                    "🟢" if status == "active" else "🟡" if status == "busy" else "⚪"
                )
