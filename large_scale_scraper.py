...
def main():
    parser = argparse.ArgumentParser(
        description="ALFA Industrial High-Speed Product Scraper"
    )
    parser.add_argument(
        "--query",
        "-q",
        type=str,
        help="Search query on marketplace (e.g. 'powerbank fast charging')",
    )
    parser.add_argument(
        "--platform",
        "-p",
        type=str,
        default="shopee",
        choices=["shopee", "tokopedia", "tiktok", "lazada", "general"],
        help="Target platform",
    )
    parser.add_argument(
        "--urls", "-u", nargs="+", help="List of URLs to scrape directly"
    )
    parser.add_argument(
        "--file",
        "-f",
    ...
    parser.add_argument(
        ...
        type=str,
        default="auto",
        choices=["auto", "fast_tls", "camoufox"],
    ...
    parser.add_argument(
        ...
        type=int,
        default=20,
        help="Max items for query search (default: 20)",
    )
    parser.add_argument(
        "--name", "-n", type=str, default="product_batch", help="Batch export name"
    )
    ...
    if args.query:
        ...
        res = fast_scraper.search_and_scrape_marketplace(
            args.query, platform=args.platform, max_items=args.limit
        )
        logger.info(f"✅ Total Scraped: {res['total_scraped']} products")
        for idx, p in enumerate(res["products"][:5], 1):
            logger.info(
                f"  [{idx}] {p['title'][:50]}... | {p['price']} | ⭐ {p['rating']}"
            )
    ...
    if not urls:
        logger.warning(
            "Mohon berikan argumen --query 'nama produk' atau --urls <url1> <url2> atau --file <file.txt>"
        )
        parser.print_help()
        sys.exit(1)
    ...
    logger.info(
        f"Total Requests: {res['total_requested']} | Sukses: {res['total_successful']} | Gagal: {res['total_failed']}"
    )
    logger.info(
        f"Waktu Total: {res['duration_seconds']}s | Kecepatan: {res['speed_urls_per_sec']} URLs/detik"
    )
