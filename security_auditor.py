...
def audit_local_host_security() -> Dict[str, Any]:
        ...
        for c in psutil.net_connections(kind="inet"):
            ...
            if laddr_ip in ("0.0.0.0", "::"):
                ...
                public_listeners.append(
                    f"{laddr_ip}:{c.laddr.port} ({proc_name}, pid {pid})"
                )
            else:
                local_listeners += 1
        add_check(
    ...
    if os.name != "nt":
        ...
        for path, max_mode in sensitive_targets:
            ...
            if mode & 0o077:
                perm_problems.append(
                    f"{os.path.basename(path)} = {oct(mode)} (harus <= {oct(max_mode)})"
                )
        if perm_checked == 0:
            add_check(
                "sensitive_permissions", True, "Tidak ada file sensitif untuk diperiksa"
            )
        else:
            ...
            add_check(
                ...
                len(perm_problems) == 0,
                f"{perm_checked} file diperiksa; "
                + ("aman" if not perm_problems else "; ".join(perm_problems)),
        ...
        add_check(
            "sensitive_permissions",
            True,
            "Izin file POSIX dilewati pada Windows (ACL digunakan)",
        )
    ...
    if os.name != "nt" and _shutil.which("systemctl"):
        try:
            ...
            failed_units = [
                ...
                if ln.strip() and ".service" in ln
            ]
            add_check(
                "failed_services",
                len(failed_units) == 0,
                (
                    f"{len(failed_units)} service user gagal: {', '.join(failed_units[:5])}"
                    if failed_units
                    else "Semua service user sehat"
                ),
                severity="MEDIUM",
            )
        ...
        add_check(
            "failed_services",
            True,
            "Pemeriksaan systemd dilewati (non-systemd OS/environment)",
        )

    # 4. Root / Primary filesystem usage
    try:
        root_path = (
            (os.path.splitdrive(os.path.abspath("."))[0] + os.sep)
            if os.name == "nt"
            else "/"
        )
...
def audit_website_security(target_url: str, timeout: int = 8) -> Dict[str, Any]:
            ...
            with socket.create_connection((hostname, port), timeout=timeout) as sock:
                with ctx.wrap_socket(sock, server_hostname=hostname) as ssock:
                    ...
                    ssl_info = {
                        ...
                        or issuer.get("commonName", "Unknown CA"),
                        "subject": subject.get("commonName", hostname),
                        "expires_on": not_after,
                    }
        except Exception as e:
            ssl_info = {"valid": False, "error": str(e), "tls_version": "N/A"}
    else:
        ssl_info = {
            "valid": False,
    ...
    if csp:
        checks["csp"] = {
            ...
            "desc": "Content Security Policy aktif",
        }
    else:
        checks["csp"] = {
            "status": "FAIL",
            "value": None,
            "desc": "Header Content-Security-Policy tidak ditemukan",
        }
        score -= 20
        findings.append(
            {
    ...
    if hsts:
        checks["hsts"] = {
            "status": "PASS",
            "value": hsts,
            "desc": "HSTS aktif memaksakan koneksi HTTPS",
        }
    else:
        checks["hsts"] = {
            "status": "FAIL",
            "value": None,
            "desc": "HSTS tidak diaktifkan",
        }
        score -= 15
        findings.append(
            {
    ...
    if xfo and xfo.upper() in ("DENY", "SAMEORIGIN"):
        checks["x_frame_options"] = {
            "status": "PASS",
            "value": xfo,
            "desc": "Anti-Clickjacking aktif",
        }
    else:
        checks["x_frame_options"] = {
            "status": "FAIL",
            "value": xfo,
            "desc": "X-Frame-Options tidak terkonfigurasi dengan aman",
        }
        score -= 15
        findings.append(
            {
    ...
    if xcto and "nosniff" in xcto.lower():
        checks["x_content_type_options"] = {
            "status": "PASS",
            "value": xcto,
            "desc": "MIME-sniffing protection aktif",
        }
    else:
        checks["x_content_type_options"] = {
            "status": "FAIL",
            "value": xcto,
            "desc": "X-Content-Type-Options: nosniff tidak ada",
        }
        score -= 10
        findings.append(
            {
    ...
    if ref:
        checks["referrer_policy"] = {
            "status": "PASS",
            "value": ref,
            "desc": "Kebijakan referrer aktif",
        }
    else:
        checks["referrer_policy"] = {
            "status": "WARN",
            "value": None,
            "desc": "Referrer-Policy tidak disetel",
        }
        score -= 5
        findings.append(
            {
    ...
    if server_hdr or x_powered:
        ...
        checks["info_leak"] = {
            "status": "WARN",
            "value": ", ".join(leaks),
            "desc": "Header membocorkan versi/teknologi server",
        }
        score -= 10
        findings.append(
            {
        ...
        checks["info_leak"] = {
            "status": "PASS",
            "value": "None",
            "desc": "Tidak ada fingerprint server yang bocor",
        }
