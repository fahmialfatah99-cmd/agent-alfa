...
def _check_style_contract(
    styles, products, reasoning, problems, replacement_targets=None
):
    ...
    for row in styles:
        ...
        if status == "supplemental" and not parent:
            problems.append(
                f"[style:{style_id}] supplemental rows require Parent Style ID"
            )
        ...
        for key in [
            ...
            *_split(row.get("Aliases", ""), "|"),
        ]:
            folded = key.casefold()
            if folded in lookup and lookup[folded] != style_id:
                problems.append(
                    f"[style] ambiguous identity '{key}' -> {lookup[folded]}, {style_id}"
                )
    ...
    for reference in sorted(set(references)):
        ...
        if not resolved_id:
            ...
            next(row for row in styles if row.get("Style ID") == resolved_id).get(
                "Status"
            )
            == "deprecated"
        ):
    ...
    for row in styles:
        ...
        if any(
            term in row.get("Framework Compatibility", "").casefold()
...
def _check_reasoning_contract(
    products, colors, reasoning, style_ids, patterns, problems
):
    semantic_sets = (
    ...
    for label, rows, key in semantic_sets:
        ...
        if len(rows) != 192:
            problems.append(
                f"[reasoning] {label} must contain exactly 192 rows; got {len(rows)}"
            )
        if duplicates:
            problems.append(
    ...
    for row in reasoning:
        ...
        for actions in rules.values():
            for action in actions:
                ...
                if prefix == "style" and value not in style_ids:
                    problems.append(
                        f"[reasoning:{category}] unknown style action '{value}'"
                    )
                if prefix == "pattern" and value not in patterns:
                    problems.append(
                        f"[reasoning:{category}] unknown pattern action '{value}'"
                    )
        ...
        if confidence:
            try:
                ...
                problems.append(
                    f"[reasoning:{category}] invalid Confidence '{confidence}'"
                )
...
def _check_color_contract(rows, problems):
    for row in rows:
        ...
        if (
            "Muted Foreground" in ratios
            and "Foreground" in ratios
...
def _check_chart_contract(rows, problems):
    for row in rows:
        ...
        if not CHART_NON_COLOR_GUIDANCE.search(fallback):
            problems.append(
                f"[chart:{data_type}] missing non-color distinction guidance"
            )
        if row.get("Interactive Level", "").strip() and "keyboard" not in fallback:
            problems.append(
                f"[chart:{data_type}] missing keyboard interaction equivalent"
            )
...
def _check_typography_contract(rows, problems):
    for row in rows:
        ...
        if not named <= families or not named <= configured:
            problems.append(
                f"[typography:{pairing}] named/imported/configured fonts differ"
            )
        ...
        if declared_weights and not declared_weights <= imported_weights:
            ...
            problems.append(
                f"[typography:{pairing}] recommended weights not imported: {missing}"
            )
...
def _check_font_catalog(rows, licenses, typography, problems):
    ...
    if not families or len(family_set) != len(families) or "" in family_set:
        problems.append(
            "[catalog:google-fonts] Family values must be non-empty and unique"
        )
    available = {}
    for row in rows:
        family = row.get("Family", "")
        if not _catalog_date(row.get("Date Added")) or not _catalog_date(
            row.get("Last Modified")
        ):
            problems.append(
                f"[catalog:google-fonts:{family}] invalid or suspicious date"
            )
    ...
    if (
        licenses.get("familyCount") != len(rows)
    ...
    for item in entries:
        if not isinstance(item, dict):
            problems.append(
                "[catalog:google-font-licenses] family entry must be an object"
            )
            continue
        name = item.get("name")
        if (
            ...
            problems.append(
                f"[catalog:google-font-licenses:{name}] invalid active family"
            )
    ...
    for item in excluded:
        ...
        if (
            ...
            or name in family_set
            or item.get("status") != "needs-review"
    ...
    for row in typography:
        ...
        for declaration in declarations:
            ...
            if family not in available:
                problems.append(
                    f"[typography:{pairing}] font absent from approved catalog: {family}"
                )
                continue
            ...
            if not weights <= available[family]:
                ...
                problems.append(
                    f"[typography:{pairing}] catalog lacks {family} weights: {missing}"
                )
...
def _check_phosphor_catalog(curated, manifest, problems):
    ...
    imports = (
        manifest.get("reactImports")
        if isinstance(manifest.get("reactImports"), dict)
        else {}
    )
    icons = manifest.get("icons")
    if (source.get("package"), source.get("version")) != (
        "@phosphor-icons/core",
        "2.1.1",
    ):
        problems.append("[catalog:phosphor] unpinned core package")
    if (source.get("reactPackage"), source.get("reactVersion")) != (
    ...
    if (
        ...
        or not _catalog_date(manifest.get("verifiedAt"))
    ...
    for item in icons:
        ...
        if (
            ...
            or item.get("clientImport")
            != f'import {{ {component} }} from "@phosphor-icons/react"'
            or item.get("ssrImport")
            != f'import {{ {component} }} from "@phosphor-icons/react/ssr"'
        ):
    ...
    for row in phosphor_rows:
        ...
        component_match = re.search(
            r"import\s*\{\s*([A-Za-z0-9]+)", row.get("Import Code", "")
        )
        component = component_match.group(1) if component_match else ""
        if names.get(name) != component:
            problems.append(
                f"[catalog:phosphor:{name}] curated icon is absent or mismatched"
            )
...
def _check_catalog_summary(summary, licenses, phosphor, problems):
    ...
    expected = {
        ...
        "stackGuidelines": sum(
            len(_read_rows(DATA_DIR / config["file"])[1])
            for config in STACK_CONFIG.values()
        ),
    }
    ...
    snapshots = (
        summary.get("snapshots") if isinstance(summary.get("snapshots"), dict) else {}
    )
    for name in (
        "google-fonts.csv",
    ...
    pending = sorted(
        (
            ...
            for item in licenses.get("excludedFamilies", [])
...
def _check_catalog_contract(domain_rows, problems):
    if (DATA_DIR / ".google-font-refresh.incomplete.json").exists():
        problems.append(
            "[catalog:google-fonts] incomplete refresh marker requires review"
        )
    ...
    if licenses:
        _check_font_catalog(
            domain_rows.get("google-fonts", []),
            licenses,
            domain_rows.get("typography", []),
            problems,
        )
    if phosphor:
...
def _check_icon_contract(rows, problems):
    for row in rows:
        ...
        if any(
            not re.search(pattern, usage, re.I) for pattern in ICON_USAGE_REQUIREMENTS
        ):
            problems.append(
                f"[icons:{icon}] incomplete contextual accessibility guidance"
            )
...
def _check_app_interface_contract(rows, problems):
    native_target = next(
        (row for row in rows if row.get("Issue") == "Touch Target Size"), {}
    )
    if not {"44pt", "48dp"} <= set(
        re.findall(r"44pt|48dp", " ".join(native_target.values()))
    ):
        problems.append(
            "[web:Touch Target Size] must distinguish iOS 44pt and Android 48dp"
        )
...
def _check_react_contract(rows, problems):
    ...
    if "useLatest" in react_text:
        problems.append(
            "[react] unqualified community useLatest guidance is not allowed"
        )
    effect_event = next(
        (row for row in rows if row.get("Issue") == "Effect Events"), {}
    )
    effect_text = " ".join(effect_event.values()).casefold()
    if not all(phrase in effect_text for phrase in ("inside effects", "dependencies")):
        problems.append(
            "[react:Effect Events] current scope/dependency guidance is required"
        )
...
def _check_stack_freshness_contract(stack, rows, problems):
    ...
    for row in rows:
        ...
        if (
            ...
            problems.append(
                f"{identity} Critical/High row requires an official Docs URL"
            )
...
def _valid_dataset_source_key(source_file, source_key, identity, problems):
    if source_file not in CORE_PROVENANCE_FILES:
        problems.append(
            f"[provenance] unknown dataset-contract sourceFile for {identity}"
        )
        return False
    ...
    if not referenced or not referenced <= row_ids:
        problems.append(
            f"[provenance] dataset scope references unknown rows for {identity}"
        )
        return False
    ...
    if not any(
        ...
        problems.append(
            f"[provenance] dataset scope must bind source fields for {identity}"
        )
        return False
    return True
...
def _valid_catalog_source_key(source_file, source_key, identity, problems):
    ...
    if (
        ...
        problems.append(
            f"[provenance] catalog sourceKey must bind snapshot and count for {identity}"
        )
        return False
    try:
...
def _check_provenance(reasoning, styles, problems):
    ...
    for index, record in enumerate(payload["records"]):
        ...
        if (
            identity[0] not in PROVENANCE_KINDS
        ...
        if not isinstance(record.get("sourceFile"), str) or not record.get(
            "sourceFile"
        ):
    ...
    if new_rows - covered:
        problems.append(
            f"[provenance] missing new reasoning rows: {', '.join(sorted(new_rows - covered))}"
        )
    ...
    core_files = {
        ...
        and any(
            isinstance(source, dict) and source.get("type") == "official"
...
def validate():
    ...
    for stack, config in STACK_CONFIG.items():
        ...
        if headers and not required <= set(headers):
            problems.append(
                f"[stack:{stack}] missing contract fields {sorted(required - set(headers))}"
            )
        for row in rows:
    ...
    for row in landing:
        ...
        if len(sections) < 2 or any(re.match(r"^\d+\.\s", part) for part in sections):
            problems.append(
                f"[landing:{row.get('Pattern Name')}] invalid Section Order delimiter"
            )
