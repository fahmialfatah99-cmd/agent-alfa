...
def lsp_find_symbol_definition(symbol_name: str, repo_path: str = "") -> Dict[str, Any]:
            ...
            for node in ast.walk(tree):
                if isinstance(
                    ...
                    if node.name == symbol_name:
                        ...
                        snippet_lines = lines[
                            max(0, start_l - 1) : min(len(lines), start_l + 15)
                        ]
                        ...
                        matches.append(
                            {
                                ...
                                "docstring": (
                                    doc.split(chr(10) + chr(10))[0] if doc else ""
                                ),
                                "snippet": snippet,
                            }
                        )
    ...
    if not matches:
        other_files = list(
            _iter_source_files(root, ["js", "ts", "jsx", "tsx", "go", "rs"])
        )
        pattern = re.compile(
            rf"(function|class|const|let|var|def|fn|type|struct)\s+{re.escape(symbol_name)}",
            re.MULTILINE,
        )
    ...
    return {
        ...
        "definitions": matches,
        "message": (
            f"Ditemukan {len(matches)} definisi untuk simbol '{symbol_name}'."
            if matches
            else f"Simbol '{symbol_name}' tidak ditemukan di {root}."
        ),
    }


def lsp_find_symbol_references(
    symbol_name: str, repo_path: str = "", limit: int = 40
) -> Dict[str, Any]:
    """
...
def lsp_analyze_module_hierarchy(
    target_file: str, repo_path: str = ""
) -> Dict[str, Any]:
    """
    ...
    if not abs_path.endswith(".py"):
        return {
            "status": "error",
            "message": "Analisis hierarki mendalam saat ini dioptimalkan untuk file Python (.py).",
        }
    ...
    try:
        ...
        return {
            "status": "error",
            "message": f"Gagal menganalisis modul {target_file}: {str(e)}",
        }
