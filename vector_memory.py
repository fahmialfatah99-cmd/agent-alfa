...
def ingest_document(
    user_id: int, title: str, content_or_path: str, category: str = "general"
) -> Dict[str, Any]:
    """
    ...
    logger.info(
        f"Ingested '{title}' ({saved_count} chunks, category: {category}) into Vector Brain for user {user_id}"
    )
    return {
        "status": "success",
...
def semantic_search(
    user_id: int, query: str, top_k: int = 5, category: Optional[str] = None
) -> List[Dict[str, Any]]:
    """
