...
WORKTREES_BASE_DIR = os.path.join(
    os.path.dirname(os.path.abspath(__file__)), ".alfa_worktrees"
)
...
def git_worktree_sandbox_create(
    task_name: str, base_branch: str = "master"
) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        if res.returncode != 0:
            ...
            if res.returncode != 0:
                return {
                    "status": "error",
                    "message": f"Gagal membuat git worktree: {res.stderr.strip()}",
                }
...
def git_worktree_sandbox_verify_and_merge(
    ...
    try:
        ...
        if not os.path.exists(worktree_path):
            return {
                "status": "error",
                "message": f"Worktree sandbox '{worktree_path}' tidak ditemukan.",
            }
        ...
        if diff_res.stdout.strip():
            ...
            if c_res.returncode != 0:
                return {
                    "status": "error",
                    "message": f"Git commit gagal: {c_res.stderr.strip()}",
                }
        ...
        m_res = _run_git(
            [
                ...
                branch_name,
                "-m",
                f"Merge sandbox branch '{branch_name}': {commit_message}",
            ],
            cwd=root,
        )
        if m_res.returncode != 0:
            ...
            return {
                "status": "error",
                "message": f"Merge konflik / gagal: {m_res.stderr.strip()}",
            }
...
def git_worktree_sandbox_list() -> Dict[str, Any]:
    ...
    try:
        ...
        return {"status": "success", "total_sandboxes": len(trees), "sandboxes": trees}
    except Exception as e:
